...
def nav02(x): # to prevent user when entering value not in the selection
    ...
    while not a.isnumeric():
        print('Your input is not in the selection.')
        a = input(f'Enter selection (1-{x}): ')
        if a.isnumeric():
            a = int(a)
            if a<1 or a>x:
...
def menu01(): # to show all data in car database
    if len(carDB)>0:
        print('*** Car Database ***')
        print("ID\t| Brand\t\t\t| Year\t| Transmission\t| Rental Price(Rp)")
...
def menu02(x): # to show selected data (by ID)
    print(carDB[x]['ID'], '\t|', carDB[x]['Brand'], '\t|',
    carDB[x]['Year'], '\t|',carDB[x]['Transmission'],'\t|',carDB[x]['Rental Price'])
...
def dict0(x,y): # to reset temporary dictionary
    global dictTemp
    dictTemp.clear()
    if x == 0:dictTemp = {'ID':'','Brand':'','Year':0,'Transmission':'','Rental Price':0}
...
def add0(x): # to update or create data
    ...
    if b == '': 
        ...
        if len(b)>20: 
            ...
            return 1 # to limit user brand input max 20 characters & alphanumeric
        else:
        ...
        # to make sure user only enter number for year and price
        if not (b.isnumeric()):
...
carDB = [
    {'ID':'TA1','Brand':'Toyota Avanza  ','Year':2015,'Transmission':'Manual','Rental Price':500000},
    {'ID':'HC1','Brand':'Honda Civic    ','Year':2020,'Transmission':'Automatic','Rental Price':1000000},
]
...
while True:
    ...
    # navigate to menu create [2]
        ...
        while True:
            ...
            userNav2 = nav01(2)
            if userNav2 == 1:
                userInput2 = input('Enter car ID to show data: ').upper()
                if userInput2 == '':print('\nALERT: ID cannot be empty')
                else:
                    ...
                    if ind2 == None:
                        ...
                        counter1 = 1
                        while counter1 < 5: # counter is used to make sure user enter all data
                            create1 = add0(counter1)
                            if create1 == None: counter1+=1
                            #elif create1 == 1:print('\nALERT: Data cannot be empty')
                        userSave = save(0)
                        if userSave == 'Y':
